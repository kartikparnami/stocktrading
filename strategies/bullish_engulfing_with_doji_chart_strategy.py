...
class BullishEngulfingWithDojiChartStrategy(BaseStrategy):

    def is_bullish_engulfing(self, symbol_data):
        # Check last body is bearish
    ...
    def run(self,
        ...
        if symbol_data.exists():
            symbol_data = DailySymbolData.objects.get(symbol=symbol,
                                                      data_souce=market_data_source,
                                                      date=date_end)
            is_last_day_bullish_engulfing = self.is_bullish_engulfing(
            ...
            if is_last_day_bullish_engulfing and is_current_day_doji and is_previously_downward and self.is_symbol_data_tradeable(symbol_data):
