...
class BullishHaramiChartStrategy(BaseStrategy):

    def is_bullish_harami(self, symbol_data):
        # Check last body
    ...
    def run(self,
        ...
        if symbol_data.exists():
            symbol_data = DailySymbolData.objects.get(symbol=symbol,
                                                      data_souce=market_data_source,
                                                      date=date_end)
            is_bullish_harami = self.is_bullish_harami(symbol_data)
            ...
            if is_bullish_harami and is_previously_downward and self.is_symbol_data_tradeable(symbol_data):
