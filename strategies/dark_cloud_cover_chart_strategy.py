...
class DarkCloudCoverChartStrategy(BaseStrategy):

    def is_dark_cloud_cover_pattern(self, symbol_data):
        # Check last body
        ...
        # Upper engulfing
        check_price = last_dsd.high_price - symbol_data.close_price
        if ((last_dsd.high_price <= symbol_data.start_price) and
        ...
        check_price = last_dsd.close_price - symbol_data.close_price
        ...
        # Lower engulfing
        check_price = symbol_data.start_price - last_dsd.low_price
        if ((last_dsd.low_price >= symbol_data.close_price) and
            (check_price >= (0.5 * (last_dsd.ohlc_size()))) and
            (check_price < (0.7 * (last_dsd.ohlc_size())))):
            self.confidence_metrics['piercing'] = 0.6
            return True
        ...
        check_price = symbol_data.start_price - last_dsd.start_price
    ...
    def run(self,
        ...
        if symbol_data.exists():
            symbol_data = DailySymbolData.objects.get(symbol=symbol,
                                                      data_souce=market_data_source,
                                                      date=date_end)
            is_dark_cloud_cover_pattern = self.is_dark_cloud_cover_pattern(symbol_data)
            ...
            if is_dark_cloud_cover_pattern and is_previously_upward and self.is_symbol_data_tradeable(symbol_data):
