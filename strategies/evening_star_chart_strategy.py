...
class EveningStarChartStrategy(BaseStrategy):
    ...
    def is_evening_star(self, symbol_data):
        # Check body two days ago
        symbol_two_days_ago = symbol_data.get_dsd_x_days_back(2)
        if not symbol_two_days_ago.is_bullish_body():
        ...
        if self.is_doji(last_dsd) and last_dsd.is_gap_up_opening():
            self.confidence_metrics['last_body'] = 1
        elif self.is_spinning_top(last_dsd) and last_dsd.is_gap_up_opening():
            self.confidence_metrics['last_body'] = 1
        else:
        ...
        if symbol_data.is_bearish_body() and symbol_data.is_gap_down_opening():
        ...
        self.extra_info['stoploss'] = max(symbol_two_days_ago.high_price, last_dsd.high_price, symbol_data.high_price)
    ...
    def run(self,
        ...
        if symbol_data.exists():
            symbol_data = DailySymbolData.objects.get(symbol=symbol,
                                                      ...
                                                      date=date_end)
            symbol_two_days_ago = symbol_data.get_dsd_x_days_back(2)
            is_evening_star = self.is_evening_star(symbol_data)
            day_difference = get_num_trading_sessions(date_start, date_end, symbol, market_data_source)
            if day_difference > 0:
                is_previously_upward = symbol_two_days_ago.is_previously_upward_trend(
                    num_days=day_difference,
                    percent_upward=60
                )
            else:
                is_previously_upward = True
            if is_evening_star and is_previously_upward and self.is_symbol_data_tradeable(symbol_data):
