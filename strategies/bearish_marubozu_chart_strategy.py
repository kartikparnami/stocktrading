...
class BearishMarubozuChartStrategy(BaseStrategy):

    def is_bearish_marubozu(self, symbol_data):
    ...
    def run(self,
        ...
        if symbol_data.exists():
            symbol_data = DailySymbolData.objects.get(symbol=symbol,
                                                      data_souce=market_data_source,
                                                      date=date_end)
            is_bearish_marubozu = self.is_bearish_marubozu(symbol_data)
            if is_bearish_marubozu and self.is_symbol_data_tradeable(symbol_data):
