...
class HammerCandlestickChartStrategy(BaseStrategy):

    def is_candlestick_paper_umbrella(self, symbol_data):
        ...
        # Low wick size check
        if low_wick_size >= (2.5 * body_size):
            self.confidence_metrics['low_wick_size'] = 1
        if (low_wick_size >= (2 * body_size)) and (low_wick_size < (2.5 * body_size)):
            self.confidence_metrics['low_wick_size'] = 0.7
        if (low_wick_size >= (1.8 * body_size)) and (low_wick_size < (2 * body_size)):
            self.confidence_metrics['low_wick_size'] = 0.5
        if (low_wick_size >= (1.5 * body_size)) and (low_wick_size < (1.8 * body_size)):
            self.confidence_metrics['low_wick_size'] = 0.2
        if low_wick_size < (1.5 * body_size):
            self.confidence_metrics['low_wick_size'] = 0
            return False

        # High wick size check
        if high_wick_size <= (0.05 * ohlc_size):
            self.confidence_metrics['high_wick_size'] = 1
        if (high_wick_size <= (0.1 * ohlc_size)) and (high_wick_size > (0.05 * ohlc_size)):
            self.confidence_metrics['high_wick_size'] = 0.7
        if (high_wick_size <= (0.15 * ohlc_size)) and (high_wick_size > (0.1 * ohlc_size)):
            self.confidence_metrics['high_wick_size'] = 0.5
        if (high_wick_size <= (0.2 * ohlc_size)) and (high_wick_size > (0.15 * ohlc_size)):
            self.confidence_metrics['high_wick_size'] = 0.3
        if high_wick_size > (0.2 * ohlc_size):
    ...
    def populate_todays_trend_confidence(self, symbol_data):
        ...
        if symbol_data.is_bearish_body():
            self.confidence_metrics['body_trend'] = 0.7
        if symbol_data.is_flat_body():
            self.confidence_metrics['body_trend'] = 0.8
    ...
    def run(self,
        ...
        if symbol_data.exists():
            symbol_data = DailySymbolData.objects.get(symbol=symbol,
                                                      data_souce=market_data_source,
                                                      date=date_end)
            hammerish_candle = self.is_candlestick_paper_umbrella(symbol_data)
            ...
            if is_previously_downward and hammerish_candle and self.is_symbol_data_tradeable(symbol_data):
